...
public class MetlTaskTiming extends BaseModel<MetlTaskTiming> {
    ...
    */
    public boolean getTimingByJobId(int jobId) {
        Long startId = getStartIdByJobId(KuConst.DATASOURCE_KETTLE, jobId);
        if(startId==null){
            return false;
        }
        ...
        for(Record record:records){
            if(StringUtil.isNotBlank(record.getStr("value_str"))){
                ...
                put(startTimingMap.get(record.getStr("code")), record.getDouble("value_num").intValue());
            }
        }
    ...
    */
    public JobEntrySpecial getStartEntryByJobId(String dbCode,int jobId) {
        Long startId = getStartIdByJobId(dbCode, jobId);
        if(startId==null){
            return null;
        }
    ...
    * @return
    */
    public Long getStartIdByJobId(String dbCode, int jobId) {
        ...
        Long startId = startIdRecord.getLong("id_jobentry");
        return startId;
    }
    ...
    */
    public int saveTimingToKettle2() throws KettleException {
        ...
        String sql = "update r_jobentry_attribute ja "
                ...
                + "where ja.ID_JOBENTRY="+startId.intValue()
                + " and ja.CODE=?";
        DbPro db = Db.use(KuConst.DATASOURCE_KETTLE);
        db.update(sql, 0,UtilConst.WHETHER_TRUE.equals(getStr("is_repeat"))?"Y":"N",
                "repeat");
        db.update(sql, Double.parseDouble(getStr("scheduler_type")),//Double.parseDouble(getStr("scheduler_type")),       		
                null,"schedulerType");
        db.update(sql, Double.parseDouble(get("interval_seconds").toString()),
                null,"intervalSeconds");
        db.update(sql, Double.parseDouble(get("interval_minutes").toString()),
                null,"intervalMinutes");
        db.update(sql, Double.parseDouble(get("hour").toString()),null,"hour");
        db.update(sql, Double.parseDouble(get("minutes").toString()),null,"minutes");
        db.update(sql, Double.parseDouble(getStr("week_day")),null,"weekDay");
        db.update(sql,Double.parseDouble(get("day_of_month").toString()),
                null,"dayOfMonth");
        return jobId;
    }
}
