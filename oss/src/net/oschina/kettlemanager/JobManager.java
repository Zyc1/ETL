...
public class JobManager extends JobEntryKettleUtilRunBase{
    ...
    * 日志文件的根路径
    */
    private static String logFileRoot = "E://temp";
    /**
    * 资源库所在数据库操作对象
    */
    ...
    */
    private static void writeEndLog(Job job) {
    	String path;
    	if(jobLogFile.isEmpty()||jobLogFile.get(job)==null)
    		path = null;
    	else 
    		path = jobLogFile.get(job).getAbsolutePath();
        kettledb.update(SQL_INSERT_END_LOG, Integer.valueOf(job.getObjectId().getId()),
                job.getJobMeta().getName(),jobStartDateMap.get(job),kettledb.getCurrentDateStr14M(),
                getJobStatus(job),path);
    }
    ...
    */
    public static String startJob(Job job){
        jobStartDateMap.put(job, kettledb.getCurrentDateStr14M());
        job.start();
